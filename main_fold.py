...
parser.add_argument('--lr_decay_step_size', type=int, default=100, help='Learning rate step size')
...
def main():
    ...
    train_set = FOLDdataset(root=args.data_path, split='train')
    val_set = FOLDdataset(root=args.data_path, split='val')
    test_set_dict = {}
    test_set_dict['test_fold'] = FOLDdataset(root=args.data_path, split='test_fold')
    test_set_dict['test_super'] = FOLDdataset(root=args.data_path, split='test_superfamily')
    test_set_dict['test_family'] = FOLDdataset(root=args.data_path, split='test_family')
    ...
    for k in test_set_dict.keys():
        test_loader_dict['test_{}_loader'.format(k)] = DataLoader(test_set_dict[k], batch_size=args.eval_batch_size, shuffle=False, num_workers=args.num_workers)
    ...
    test_loss_dict = {}
    for k in test_set_dict.keys():
        test_loss_dict['{}_acc_at_best_val'.format(k)] = 0
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            for k in test_set_dict.keys():
                _, test_acc = evaluation(args, model, test_loader_dict['test_{}_loader'.format(k)], device)
                test_loss_dict['{}_acc_at_best_val'.format(k)] = test_acc
        ...
        wandb.log({
            ...
            **{f'{k}': v for k, v in test_loss_dict.items()},
        })
